...
const EditTutorial = ({ id, topic, description, photo, video }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch(BackendUrl + `weighttraining/update/${id}`, {
            ...
            if (response.status === 200) {
                setSuccessMessage('Done. Tutorial detais edited successfully');
                setErrorMessage('');
    ...
    return (
        <div>
            ...
            <Button sx={{ color: 'white' }} size="small" className='editbutton' onClick={openModal}>Edit</Button>
            {successMessage ? (
